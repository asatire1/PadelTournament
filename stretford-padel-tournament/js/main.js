...
function SettingsTab() {
    ...
    if (state.settingsSubTab === 'players') {
        ...
        content = `
            <div class="space-y-6">
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <p class="text-sm text-gray-600 mb-2">Customise match pairings for each round. Enter player numbers (1-${CONFIG.TOTAL_PLAYERS}).</p>
                                        ...
                                        ${state.fixtures[round].map((match, matchIdx) => `
                                            <div class="flex items-center gap-2 bg-gray-50 p-3 rounded flex-wrap">
                                                ...
                                                <div class="flex items-center gap-1">
                                                    <input type="number" min="1" max="${CONFIG.TOTAL_PLAYERS}" value="${match.team1[0]}" class="fixture-input" id="r${round}m${matchIdx}t1p1" onchange="handleFixtureChange(${round}, ${matchIdx}, 't1p1', ${match.team1[0]}, this.value)" ${!isUnlocked ? 'onclick="checkPasscode(); this.blur(); return false;"' : ''} />
                                                    <span class="text-gray-400">&</span>
                                                    <input type="number" min="1" max="${CONFIG.TOTAL_PLAYERS}" value="${match.team1[1]}" class="fixture-input" id="r${round}m${matchIdx}t1p2" onchange="handleFixtureChange(${round}, ${matchIdx}, 't1p2', ${match.team1[1]}, this.value)" ${!isUnlocked ? 'onclick="checkPasscode(); this.blur(); return false;"' : ''} />
                                                </div>
                                                <span class="text-gray-400 font-bold">vs</span>
                                                <div class="flex items-center gap-1">
                                                    <input type="number" min="1" max="${CONFIG.TOTAL_PLAYERS}" value="${match.team2[0]}" class="fixture-input" id="r${round}m${matchIdx}t2p1" onchange="handleFixtureChange(${round}, ${matchIdx}, 't2p1', ${match.team2[0]}, this.value)" ${!isUnlocked ? 'onclick="checkPasscode(); this.blur(); return false;"' : ''} />
                                                    <span class="text-gray-400">&</span>
                                                    <input type="number" min="1" max="${CONFIG.TOTAL_PLAYERS}" value="${match.team2[1]}" class="fixture-input" id="r${round}m${matchIdx}t2p2" onchange="handleFixtureChange(${round}, ${matchIdx}, 't2p2', ${match.team2[1]}, this.value)" ${!isUnlocked ? 'onclick="checkPasscode(); this.blur(); return false;"' : ''} />
                                                </div>
                                            </div>
                                        `).join('')}
                                    </div>
        ...
        content = `
            <div class="space-y-6">
                <div class="bg-white rounded-lg shadow p-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-3">🔄 Reset to JSON Defaults</h3>
                    <p class="text-sm text-gray-600 mb-4">Load fresh player names, ratings, and fixtures from your JSON files. This will replace all current data with defaults from data/players.json, data/fixtures.json, and data/match-names.json.</p>
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
                        <p class="text-sm text-yellow-800">⚠️ This will create an automatic backup before resetting.</p>
                    </div>
                    <button onclick="resetToJsonDefaults()" class="px-6 py-3 bg-purple-500 hover:bg-purple-600 text-white rounded-lg font-medium">Reset to JSON Defaults</button>
                </div>
                <div class="bg-white rounded-lg shadow p-6">
