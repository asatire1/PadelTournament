...
function ResultsMatrixTab() {
    // Build matrix: players as rows, rounds as columns (13 rounds)
    const playerRoundPoints = {}; // playerId -> array of 13 round total points
        ...
        for (let matchIdx = 0; matchIdx < CONFIG.MATCHES_PER_ROUND; matchIdx++) {
            ...
            allPlayers.forEach(playerId => {
                if (score.team1Score === null || score.team2Score === null) {
                    // Match not played yet - mark as pending
                    if (playerRoundPoints[playerId][round - 1] === null) {
                        playerRoundPoints[playerId][round - 1] = '-';
                    }
                } else {
                    // Match completed - add player's points to their round total
    ...
    return `
        <div class="space-y-6">
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <div class="text-sm text-blue-800">Shows total points scored by each player in each round (${CONFIG.TOTAL_ROUNDS} rounds total). Each player plays once per round.</div>
            </div>
